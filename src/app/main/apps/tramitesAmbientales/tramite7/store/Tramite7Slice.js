...
export const getDocumentType = () => async (dispatch) => {
  try {
    ...
    console.log('*** REDUX -> getDocumentType ***', err.response)
...
export const getStates = () => async (dispatch) => {
  try {
    ...
    console.log('*** REDUX -> getStates ***', err.response)
    await dispatch(showMessage({ message: `${err}`, variant: 'error' }))
    return false
  }
};
...
export const getCities = () => async (dispatch) => {
  try {
    ...
    console.log('*** REDUX -> getCities ***', err.response)
    await dispatch(showMessage({ message: `${err}`, variant: 'error' }))
    return false
  }
};
...
export const getSolicitante = (id) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/datos_solicitante/${id}`
    const result = await axios.get(URL);
    const solicitud = result.data;
    const data = {
      ...
      expeditionPlaceRep: { value: solicitud.fk_repapo_ciudad_expedicion.cty_id, label: solicitud.fk_repapo_ciudad_expedicion.cty_name },
      addressRep: solicitud.tds_repapo_direccion,
      cityRep: { value: solicitud.fk_repapo_ciudad.cty_id, label: solicitud.fk_repapo_ciudad.cty_name },
...
export const saveDataSolicitante = (body) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/datos_solicitante`;
    const result = await axios.post(URL, body);
...
export const updateDataSolicitante = (body, solId) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/datos_solicitante/${solId}`;
...
export const getInfoGeneral = (id) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/informacion_general/${id}`
    const result = await axios.get(URL);
    if (!_.isEmpty(result.data)) {
      const solicitud = result.data;
      const data = {
        municipio: { value: solicitud.fk_cities.cty_id, label: solicitud.fk_cities.cty_name },
        departamento: { value: solicitud.fk_cities.fk_states.st_id, label: solicitud.fk_cities.fk_states.st_name },
...
export const saveDataInfoGeneral = (body) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/informacion_general`;
    const result = await axios.post(URL, body);
    await dispatch(showMessage({ message: 'Exito', variant: 'success' }))
    return true
  } catch (err) {
...
export const updateDataInfoGeneral = (body, solId) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/informacion_general/${solId}`;
    const result = await axios.put(URL, body);
    await dispatch(showMessage({ message: 'Exito', variant: 'success' }))
    return true
  } catch (err) {
...
export const getPuntoCaptacion = (id) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/info_punto_captacion/${id}`
    const result = await axios.get(URL);
    if (!_.isEmpty(result.data)) {
      const solicitud = result.data;
      const data = {
        tipoPuntoAgua: solicitud.tip_tipo_punto_agua,
        otroCual: solicitud.tip_otro_punto_agua,
        puntoAgua: solicitud.tip_nuevo_existente,
        nombrePunto: solicitud.tip_nombre,
        servidumbre: solicitud.tip_requiere_servidumbre,
        numeroExpedicion: solicitud.tip_num_permiso,
        fechaExpedicion: solicitud.tip_fecha_permiso,
        infoCoordenada: solicitud.tip_coordenadas,
        anexaInfoExploracion: solicitud.tip_anexa_info_exploracion,
        infoExploracion: solicitud.tip_info_exploracion,
      }
...
export const saveDataPuntoCaptacion = (body) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/info_punto_captacion`;
    const result = await axios.post(URL, body);
    await dispatch(showMessage({ message: 'Exito', variant: 'success' }))
    return true
  } catch (err) {
...
export const updateDataPuntoCaptacion = (body, solId) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/info_punto_captacion/${solId}`;
    const result = await axios.put(URL, body);
    await dispatch(showMessage({ message: 'Exito', variant: 'success' }))
    return true
  } catch (err) {
...
export const getDemandaUso = (id) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/demanda_uso/${id}`
    const result = await axios.get(URL);
    if (!_.isEmpty(result.data)) {
      const solicitud = result.data;
      const data = {
...
export const saveDataDemandaUso = (body) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/demanda_uso`;
    const payload = {
      ...body,
...
export const updateDataDemandaUso = (body, solId) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/demanda_uso/${solId}`;
    const payload = {
      ...body,
      tdu_otrofin_tipo_actividad: body.tipoActividad,
      tdu_descripcion_otrouso: body.descripcionFinUso,
...
export const getFileAnexos = (id) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/anexos_doc/${id}`
    const result = await axios.get(URL);
    if (!_.isEmpty(result.data)) {
      const solicitud = result.data;
      const data = {
        ...
        permisoProsExp: solicitud.permisoProsExp,
        modificacionAg: solicitud.modificacionAg,
...
export const createFileAnexo = (payload) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/anexos_doc`;
    const result = await axios.post(URL, payload);
    await dispatch(showMessage({ message: 'Exito', variant: 'success' }))
    return true
  } catch (err) {
...
export const updateAnexo = (body, solId) => async (dispatch) => {
  try {
    const URL = `${API.baseUrl}/${API.endpoints.solicitud_seven}/anexos/${solId}`;
    const result = await axios.put(URL, body);
    await dispatch(showMessage({ message: 'Exito', variant: 'success' }))
    return true
  } catch (err) {
...
export const { setLoading, setRedirect, setDocumentoAnexos, setDataTipoDocumento, setDataCities, setDataStates, setSolicitud,
  setDataSolicitante, setDataInfoGeneral, setDataPuntoCaptacion, setDataDemandaUso, setDataAnexoArchivo
